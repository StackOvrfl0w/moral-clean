...
import { Building2, Camera, Globe, Mail, MapPin, Phone, Share2 } from "lucide-react";
...
export function Footer({ settings }: FooterProps) {
  const socialLinks = [
    { label: "Facebook", href: settings.social_facebook, Icon: Globe },
    { label: "Instagram", href: settings.social_instagram, Icon: Camera },
    { label: "LinkedIn", href: settings.social_linkedin, Icon: Building2 },
    { label: "YouTube", href: settings.social_youtube, Icon: Share2 },
  ...
  const tagline = settings.business_tagline || "A trusted provider of commercial cleaning equipment and expert repair services across Pakistan. Supplying high-performance machinery ranging from industrial floor cleaners to high-pressure washers. We deliver end-to-end operational support to demanding sectors like pharmaceutical facilities, hospitals, and hotels, ensuring flawless hygiene standards without avoidable downtime.";
        ...
        <nav aria-label="Footer product categories">
          ...
          <ul className="mt-5 space-y-3 text-sm text-brand-navy/80">
            {productCategories.map((category) => (
              <li key={category}>
                <Link href={categoryHref(category)} className="transition-colors hover:text-brand-navy">
                  {category}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <address className="mt-5 space-y-4 text-sm not-italic text-brand-navy/80">
            <p className="flex gap-3">
              <MapPin className="mt-0.5 size-4 shrink-0 text-brand-navy" aria-hidden="true" />
              <span>{address}</span>
            </p>
            <p>
              <Link href={`tel:+${phoneDigits}`} className="flex gap-3 transition-colors hover:text-brand-navy">
                <Phone className="mt-0.5 size-4 shrink-0 text-brand-navy" aria-hidden="true" />
                <span>{phone}</span>
              </Link>
            </p>
            <p>
              <Link href={`mailto:${email}`} className="flex gap-3 transition-colors hover:text-brand-navy">
                <Mail className="mt-0.5 size-4 shrink-0 text-brand-navy" aria-hidden="true" />
                <span>{email}</span>
              </Link>
            </p>
            {hours ? <p className="text-xs text-brand-navy/65">{hours}</p> : null}
          </address>
        </div>
      </div>
