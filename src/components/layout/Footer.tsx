...
export function Footer({ settings }: FooterProps) {
  ...
  return (
    <footer className="text-[#0a2540]" style={{ background: "linear-gradient(to right, #00defc, #00a8ff)" }}>
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-4 lg:px-8">
        <div>
          <Link href="/" className="flex items-center gap-3" aria-label="Moral Clean home">
            <span className="font-etna text-lg font-bold uppercase tracking-wide" style={{ color: "#0a2540" }}>
              Moral Clean
            </span>
          </Link>
          <p className="mt-5 max-w-sm text-sm leading-6 text-[#0a2540]/80">
            {tagline}
          </p>
        </div>

        <nav aria-label="Footer quick links">
          <h2 className="font-display text-sm font-semibold uppercase tracking-wide text-[#0a2540]">
            Quick Links
          </h2>
          <ul className="mt-5 space-y-3 text-sm text-[#0a2540]/80">
            {quickLinks.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="transition-colors hover:text-[#0a2540]">
                  {link.label}
                </Link>
              </li>
            ))}
        ...
        <nav aria-label="Footer product categories">
          ...
          <ul className="mt-5 space-y-3 text-sm text-[#0a2540]/80">
            {productCategories.map((category) => (
              <li key={category}>
                <Link href={categoryHref(category)} className="transition-colors hover:text-[#0a2540]">
                  {category}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <address className="mt-5 space-y-4 text-sm not-italic text-[#0a2540]/80">
            <p className="flex gap-3">
              <MapPin className="mt-0.5 size-4 shrink-0 text-[#0a2540]" aria-hidden="true" />
              <span>{address}</span>
            </p>
            <p>
              <Link href={`tel:+${phoneDigits}`} className="flex gap-3 transition-colors hover:text-[#0a2540]">
                <Phone className="mt-0.5 size-4 shrink-0 text-[#0a2540]" aria-hidden="true" />
                <span>{phone}</span>
              </Link>
            </p>
            <p>
              <Link href={`mailto:${email}`} className="flex gap-3 transition-colors hover:text-[#0a2540]">
                <Mail className="mt-0.5 size-4 shrink-0 text-[#0a2540]" aria-hidden="true" />
                <span>{email}</span>
              </Link>
            </p>
            {hours ? <p className="text-xs text-[#0a2540]/65">{hours}</p> : null}
          </address>
        </div>
      </div>

      <div className="border-t border-[#0a2540]/15">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 text-sm text-[#0a2540]/70 sm:flex-row sm:items-center sm:justify-center sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-3" aria-label="Social links">
              {socialLinks.map(({ label, href, Icon }) => (
                <Link
                  ...
                  className="flex size-9 items-center justify-center rounded-full border border-[#0a2540]/30 text-[#0a2540]/70 transition-all duration-200 hover:border-[#0a2540] hover:text-[#0a2540]"
                  aria-label={label}
                >
